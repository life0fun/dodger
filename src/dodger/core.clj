(ns dodger.core
  ...
  (:require [dodger.elastic.es :as es]
            [dodger.vwfeature :as vwfeature]
            [dodger.incanter.plot :as plot])
  (:gen-class :main true))
...
(defn gen-feature [args]
  ...
  (let [now (clj-time.local/local-now)
        ...
        vmfeature (vwfeature/gen-feature es/dodger-data-index-name timestr backhours)] ; back 24 hour
...
(defn train [args]
  ...
  (let [now (clj-time.local/local-now)
    ...
    (vwfeature/train-model datfile evtfile mdlfile)))
...
(defn -main [& args]
  ...
  (case (first args)
    ...
    ; lein-2 run gen-feature "9/28/2005 14:00" 3
